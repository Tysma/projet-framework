...
import { checkConnected, isConnected } from '../components/etatConnexion';
...
onMounted(() => {
  checkConnected();
  console.log("Current isConnected:", isConnected.value);
...
async function loadCards() {
  ...
  if (response.ok) {
    ...
    console.error('Erreur lors de la récupération des chaussures');
  }
}
...
const saveCard = async () => {
  try {
    ...
    if (response.ok) {
      loginMessage.value = 'Chaussure ajoutée avec succès';
    ...
    <div class="flex flex-col justify-center h-full">
      <div class="w-full max-w-2xl mx-auto bg-white shadow-lg rounded-sm border border-gray-200">
        <header class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <p class="text-center text-xl font-light text-green-500">{{ loginMessage }}</p>
