...
import { setConnected } from '../components/etatConnexion';
...
async function login() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      setConnected(true);
      loginMessage.value = "Connexion réussie. Vous allez être redirigé vers l'inventaire dans 3 secondes.";
      setTimeout(() => {
        router.push({ path: '/inventaire' });
      }, 3000);
    } else {
      ...
      <div class="w-full rounded-xl shadow dark:border md:mt-0 sm:max-w-md xl:p-0 bg-gray-800 ">
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <div class="space-y-4 md:space-y-6">
            ...
            <p class="text-center text-sm font-light text-white">{{ loginMessage }}</p>
